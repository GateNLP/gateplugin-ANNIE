...
/**
 * This class implements a Finite State Machine transition. A transition is
 * owned by a gate.fsm.State object and contains set of restrictions and a
 * reference to the next state that will be accessed after consuming a set of
 * input symbols according to the restrictions. A transition can also hold
 * information about the label that should be bound to the symbols (annotations)
 * consumed during the state transition.
 */
// >>> DAM
/*
...
public class Transition implements Serializable, Comparable<Transition> {
  ...
  /**
   * Default constructor. Creates a new transition with a new unique index. This
   * constructor should be called by all other constructors.
   */
  public Transition() {
   ...
   * 
   * @param constraints
   *          the set on constraints associated to this transition
   * @param state
   ...
   * list of labels to be bound with the recognized input symbols (aka
   * annotations).
   */
  public Transition(BasicPatternElement constraints, State state,
  ...
  /**
   * Creates a new transition to the given State with the same bindings as this
   * one.
   */
  ...
  /**
   * Returns a boolean value indicating whether this Transition has any
   * constraints on it.
   */
  ...
  /**
   * Returns true if all the constraints on this transition are satisfied by the
   * given Annotations, false otherwise. The given Annotations should be the set
   * of Annotations beginning at a single point in the document.
   */
  public boolean satisfiedBy(Annotation[] coIncidentAnnos) {
    Constraint[] allConstraints = getConstraints().getConstraints();

    processAllConstraints: for(int i = 0; i < allConstraints.length; i++) {
      ...
      for(int j = 0; j < coIncidentAnnos.length; j++) {
        if(c.matches(coIncidentAnnos[j], null)) {
          // One of these puppies being satisfied invalidates the whole
          // transition
  ...
  /**
   * Returns a boolean value indicating whether this Transition deals with
  ...
  /**
   * Returns a shorter description that toSting(). Actually, it returns the
   * unique index in String form.
   */
  ...
  /**
   * The unique index of this transition. This value is not used by any of the
   * algorithms. It is only provided as a convenient method of identifying the
   * transitions in textual representations (toString() and GML related methods)
   */
  private int myIndex;
